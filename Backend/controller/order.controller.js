...
export const getOrderByStatusController = async (req, res) => {
  try {
    const { id: userId } = req?.user;
    ...
    if (!orders.length) {
      return sendErrorResponse(res, 404, "No orders found for the given criteria.");
    }

    return sendSuccessResponse(
      res,
      `Orders fetched successfully${status ? ` for status: ${status}` : ""}.`,
      {
        total: orders.length,
        orders
      }
    ...
    console.error(error);
    return sendErrorResponse(res, 500, "Error during fetching orders by status.", error);
  }
};

export const getOrderSellerByStatus = async (req, res) => {
  try {
    const { _id: sellerId } = req?.user;
    const { status } = req?.query;
